...
def naiveSoftmaxLossAndGradient(
    ...
    v_c, W, target = centerWordVec, outsideVectors, outsideWordIdx
    v_c = v_c.reshape((v_c.shape[0], 1))
    # v_c（V, 1) W.shape == (N, V)）
    uv = np.dot(W, v_c)
    preds = softmax(uv.reshape(-1)).reshape(-1, 1) # (N, 1)
...
def negSamplingLossAndGradient(
    ...
    indices = [outsideWordIdx] + negSampleWordIndices #1 + k,

    ### YOUR CODE HERE
    W = np.zeros((len(indices), outsideVectors.shape[1])) #(1 + K,V)
    for i in range(len(indices)):
        W[i] = outsideVectors[indices[i]]

    v_c = centerWordVec
    v_c = v_c.reshape((a.shape[0], 1)) # V , 1
    uv = np.dot(W,v_c) #(K + 1, 1) !!!
